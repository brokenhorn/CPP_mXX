...
{
 std::cout << "Constructor(ClapTrap) is called with default parametrs" << std::endl;
}

ClapTrap::ClapTrap(std::string name): _name(name), _hitpoints(10), _energy_points(10), _attack_damage(0)
{
	std::cout << "Constructor(ClapTrap) is called with custom parametrs" << std::endl;
}
...
{
	std::cout << "Assigment operator(ClapTrap) is called" << std::endl;
